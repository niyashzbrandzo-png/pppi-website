...
export default function LeadershipSection({ setActivePage }) {
  ...
  return (
    <section className="section-padding" id="leadership" style={{ backgroundColor: 'var(--bg-secondary)' }} aria-label="PPPI Leadership Directory">
      <div className="container">
        <div className="section-header">
          ...
          <p className="section-subtitle">
            Dedicated administrative leaders and grassroots coordinators steering PPPI's digital transformation across India.
          </p>

          {/* Filter Buttons */}
          <div className="filter-tabs" style={{ marginTop: '1.5rem' }} role="tablist" aria-label="Leadership Categories">
            {categories.map((cat) => (
              <button
                key={cat}
                role="tab"
        ...
        <div className="cards-grid-3">
          {filteredTeam.map((member) => (
            <div key={member.id} className="leader-card">
              <img
                src={member.image}
                alt={`${member.name} - ${member.role} of Pasha People Party of India`}
              ...
              <div className="leader-info">
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border-color)', paddingTop: '0.85rem' }}>
                  <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 600 }}>
                    <i className="fa-solid fa-location-dot" style={{ color: 'var(--color-saffron-dark)' }} aria-hidden="true"></i> Verified Officer
                  </span>
                  <button
